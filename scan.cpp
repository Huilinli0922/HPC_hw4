...
void scan(long* prefix_sum, const long* A, long* correction, int rank, int size, long n, long* B2) {
    
    prefix_sum[0] = A[0]; 
    for (int j = 1; j < n; j++){
        prefix_sum[j] = prefix_sum[j - 1] + A[j];     }
    correction[rank] = prefix_sum[n-1];
...
int main(int argc, char* argv[]) {
    ...
    s2 = MPI_Wtime();
        ...
        scan(B1, receive, correction, rank, size, chunksize, B2);
    e2 = MPI_Wtime();
